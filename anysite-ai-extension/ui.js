...
async function checkProfileStatus() {
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.BACKEND_URL}/profile/status?auth_token=${authToken}`, {
    ...
    if (response.ok) {
      ...
      if (data.missing_sections && data.missing_sections.length > 0) {
        appendMessage("ai", `**Profile Incomplete**<br>Please complete the following sections from the top menu: ${data.missing_sections.join(", ").replace(/_/g, " ")}`);
      }
    }
  } catch (err) {
...
document.getElementById("new-chat-btn").addEventListener("click", () => {
  const log = document.getElementById("log");
  log.innerHTML = '<div class="msg ai">New conversation started. How can I help you today?</div>';
});
...
async function renderForm(tableName) {
  ...
  if (authToken) {
    try {
      ...
      const res = await fetch(`${CONFIG.BACKEND_URL}/profile/${tableName}?auth_token=${authToken}`, {
        headers: fetchHeaders
      });
      if (res.ok) {
